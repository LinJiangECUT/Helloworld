...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        # add & norm
        residual = self.beta1*src + self.dropout1(output)
        if not self.normalize_before:
            output = self.norm1(residual)

        if self.normalize_before:
            output = self.norm2(residual)
        else:
            output = residual
        output = self.pos_ffn(output)

        # add & norm
        output = self.beta2*residual + self.dropout2(output) + self.alpha*src
        if not self.normalize_before:
